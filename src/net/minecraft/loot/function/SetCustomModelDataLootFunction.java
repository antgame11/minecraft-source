...
extends ConditionalLootFunction {
    ...
    public static final MapCodec<SetCustomModelDataLootFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> SetCustomModelDataLootFunction.addConditionsField(instance).and(instance.group(ListOperation.Values.createCodec(LootNumberProviderTypes.CODEC, Integer.MAX_VALUE).optionalFieldOf("floats").forGetter(lootFunction -> lootFunction.floats), ListOperation.Values.createCodec(Codec.BOOL, Integer.MAX_VALUE).optionalFieldOf("flags").forGetter(lootFunction -> lootFunction.flags), ListOperation.Values.createCodec(Codec.STRING, Integer.MAX_VALUE).optionalFieldOf("strings").forGetter(lootFunction -> lootFunction.strings), ListOperation.Values.createCodec(COLOR_CODEC, Integer.MAX_VALUE).optionalFieldOf("colors").forGetter(lootFunction -> lootFunction.colors))).apply((Applicative<SetCustomModelDataLootFunction, ?>)instance, SetCustomModelDataLootFunction::new));
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        CustomModelDataComponent lv = stack.getOrDefault(DataComponentTypes.CUSTOM_MODEL_DATA, CustomModelDataComponent.DEFAULT);
        stack.set(DataComponentTypes.CUSTOM_MODEL_DATA, new CustomModelDataComponent(SetCustomModelDataLootFunction.apply(this.floats, lv.floats(), (T provider) -> Float.valueOf(provider.nextFloat(context))), SetCustomModelDataLootFunction.apply(this.flags, lv.flags()), SetCustomModelDataLootFunction.apply(this.strings, lv.strings()), SetCustomModelDataLootFunction.apply(this.colors, lv.colors(), (T provider) -> provider.nextInt(context))));
        return stack;
    }
}
