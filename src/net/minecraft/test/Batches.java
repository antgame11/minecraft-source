...
public class Batches {
    ...
    public static List<GameTestBatch> batch(Collection<RegistryEntry.Reference<TestInstance>> instances, Decorator decorator, ServerWorld world) {
        ...
        return map.entrySet().stream().flatMap(entry -> {
            RegistryEntry lv = (RegistryEntry)entry.getKey();
            List list = (List)entry.getValue();
            return Streams.mapWithIndex(Lists.partition(list, 50).stream(), (states, index) -> Batches.create(states, lv, (int)index));
        }).toList();
    }
    ...
    public static TestRunContext.Batcher batcher(int batchSize) {
        return states -> {
            ...
            return map.entrySet().stream().flatMap(entry -> {
                RegistryEntry lv = (RegistryEntry)entry.getKey();
                List list = (List)entry.getValue();
                return Streams.mapWithIndex(Lists.partition(list, batchSize).stream(), (states, index) -> Batches.create(List.copyOf(states), lv, (int)index));
            }).toList();
        };
    }
