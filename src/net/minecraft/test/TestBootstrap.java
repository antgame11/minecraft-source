...
public class TestBootstrap {
    ...
    public static void run(String[] args, Consumer<String> universeCallback) throws Exception {
        ...
        if (optionSet.valueOf(VERIFY).booleanValue() && !optionSet.has(TESTS)) {
            LOGGER.error("Please specify a test selection to run the verify option. For example: --verify --tests example:test_something_*");
            System.exit(-1);
        }
        LOGGER.info("Running GameTestMain with cwd '{}', universe path '{}'", (Object)System.getProperty("user.dir"), (Object)optionSet.valueOf(UNIVERSE));
        ...
        LevelStorage.Session lv = LevelStorage.create(Paths.get(string, new String[0])).createSessionWithoutSymlinkCheck(DEFAULT_WORLD);
        ResourcePackManager lv2 = VanillaDataPackProvider.createManager(lv);
        MinecraftServer.startServer(thread -> TestServer.create(thread, lv, lv2, TestBootstrap.get(optionSet, TESTS), optionSet.has(VERIFY)));
    }
