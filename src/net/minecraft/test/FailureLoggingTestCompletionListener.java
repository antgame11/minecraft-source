...
implements TestCompletionListener {
    ...
    public void onTestFailed(GameTestState test) {
        ...
        if (test.isRequired()) {
            LOGGER.error("{} failed at {}! {}", test.getId(), string, Util.getInnermostMessage(test.getThrowable()));
        } else {
            LOGGER.warn("(optional) {} failed at {}. {}", test.getId(), string, Util.getInnermostMessage(test.getThrowable()));
        }
    }
