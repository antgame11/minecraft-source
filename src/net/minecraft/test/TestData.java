...
public record TestData<EnvironmentType>(EnvironmentType environment, Identifier structure, int maxTicks, int setupTicks, boolean required, BlockRotation rotation, boolean manualOnly, int maxAttempts, int requiredSuccesses, boolean skyAccess) {
    public static final MapCodec<TestData<RegistryEntry<TestEnvironmentDefinition>>> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)TestEnvironmentDefinition.ENTRY_CODEC.fieldOf("environment")).forGetter(TestData::environment), ((MapCodec)Identifier.CODEC.fieldOf("structure")).forGetter(TestData::structure), ((MapCodec)Codecs.POSITIVE_INT.fieldOf("max_ticks")).forGetter(TestData::maxTicks), Codecs.NON_NEGATIVE_INT.optionalFieldOf("setup_ticks", 0).forGetter(TestData::setupTicks), Codec.BOOL.optionalFieldOf("required", true).forGetter(TestData::required), BlockRotation.CODEC.optionalFieldOf("rotation", BlockRotation.NONE).forGetter(TestData::rotation), Codec.BOOL.optionalFieldOf("manual_only", false).forGetter(TestData::manualOnly), Codecs.POSITIVE_INT.optionalFieldOf("max_attempts", 1).forGetter(TestData::maxAttempts), Codecs.POSITIVE_INT.optionalFieldOf("required_successes", 1).forGetter(TestData::requiredSuccesses), Codec.BOOL.optionalFieldOf("sky_access", false).forGetter(TestData::skyAccess)).apply((Applicative<TestData, ?>)instance, TestData::new));
