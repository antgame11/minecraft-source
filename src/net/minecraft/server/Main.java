...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                lv10 = (SaveLoader)Util.waitAndApply(applyExecutor -> SaveLoading.load(lv9, arg2 -> {
                    RegistryWrapper.Impl lv = arg2.dimensionsRegistryManager().getOrThrow(RegistryKeys.DIMENSION);
                    if (dynamic2 != null) {
                        ParsedSaveProperties lv2 = LevelStorage.parseSaveProperties(dynamic2, arg2.dataConfiguration(), (Registry<DimensionOptions>)lv, arg2.worldGenRegistryManager());
                        return new SaveLoading.LoadContext<SaveProperties>(lv2.properties(), lv2.dimensions().toDynamicRegistryManager());
                    }
                    LOGGER.info("No existing world data, creating new world");
                    return Main.createWorld(lv, arg2, (Registry<DimensionOptions>)lv, optionSet.has(optionSpec3), optionSet.has(optionSpec4));
            ...
            if (optionSet.has(optionSpec5) || bl2) {
                Main.forceUpgradeWorld(lv5, lv12, Schemas.getFixer(), optionSet.has(optionSpec6), () -> true, lv11, bl2);
            }
            lv5.backupLevelDataFile(lv11, lv12);
            final MinecraftDedicatedServer lv13 = MinecraftServer.startServer(thread -> {
                boolean bl;
                MinecraftDedicatedServer lv = new MinecraftDedicatedServer((Thread)thread, lv5, lv8, lv10, lv, Schemas.getFixer(), lv3);
    ...
    private static SaveLoading.LoadContext<SaveProperties> createWorld(ServerPropertiesLoader arg, SaveLoading.LoadContextSupplierContext arg2, Registry<DimensionOptions> arg3, boolean bl, boolean bl2) {
        DimensionOptionsRegistryHolder lv3;
        GeneratorOptions lv2;
        LevelInfo lv;
        if (bl) {
            lv = MinecraftServer.DEMO_LEVEL_INFO;
            lv2 = GeneratorOptions.DEMO_OPTIONS;
            lv3 = WorldPresets.createDemoOptions(arg2.worldGenRegistryManager());
        } else {
            ServerPropertiesHandler lv4 = arg.getPropertiesHandler();
            lv = new LevelInfo(lv4.levelName, lv4.gameMode.get(), lv4.hardcore, lv4.difficulty.get(), false, new GameRules(arg2.dataConfiguration().enabledFeatures()), arg2.dataConfiguration());
            lv2 = bl2 ? lv4.generatorOptions.withBonusChest(true) : lv4.generatorOptions;
            lv3 = lv4.createDimensionsRegistryHolder(arg2.worldGenRegistryManager());
        }
        DimensionOptionsRegistryHolder.DimensionsConfig lv5 = lv3.toConfig(arg3);
        Lifecycle lifecycle = lv5.getLifecycle().add(arg2.worldGenRegistryManager().getLifecycle());
        return new SaveLoading.LoadContext<SaveProperties>(new LevelProperties(lv, lv2, lv5.specialWorldProperty(), lifecycle), lv5.toDynamicRegistryManager());
    }
    ...
    private static void forceUpgradeWorld(LevelStorage.Session session, SaveProperties saveProperties, DataFixer dataFixer, boolean eraseCache, BooleanSupplier continueCheck, DynamicRegistryManager registries, boolean recreateRegionFiles) {
        ...
        try (WorldUpdater lv = new WorldUpdater(session, dataFixer, saveProperties, registries, eraseCache, recreateRegionFiles);){
            Text lv2 = null;
            while (!lv.isDone()) {
                int i;
