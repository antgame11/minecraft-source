...
public class RaidCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("raid").requires(CommandManager.requirePermissionLevel(3))).then(CommandManager.literal("start").then((ArgumentBuilder<ServerCommandSource, ?>)CommandManager.argument("omenlvl", IntegerArgumentType.integer(0)).executes(context -> RaidCommand.executeStart((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "omenlvl")))))).then(CommandManager.literal("stop").executes(context -> RaidCommand.executeStop((ServerCommandSource)context.getSource())))).then(CommandManager.literal("check").executes(context -> RaidCommand.executeCheck((ServerCommandSource)context.getSource())))).then(CommandManager.literal("sound").then((ArgumentBuilder<ServerCommandSource, ?>)CommandManager.argument("type", TextArgumentType.text(registryAccess)).executes(context -> RaidCommand.executeSound((ServerCommandSource)context.getSource(), TextArgumentType.parseTextArgument(context, "type")))))).then(CommandManager.literal("spawnleader").executes(context -> RaidCommand.executeSpawnLeader((ServerCommandSource)context.getSource())))).then(CommandManager.literal("setomen").then((ArgumentBuilder<ServerCommandSource, ?>)CommandManager.argument("level", IntegerArgumentType.integer(0)).executes(context -> RaidCommand.executeSetOmen((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "level")))))).then(CommandManager.literal("glow").executes(context -> RaidCommand.executeGlow((ServerCommandSource)context.getSource()))));
    }
    ...
    private static int executeStart(ServerCommandSource source, int level) throws CommandSyntaxException {
        ...
        if (lv.getEntityWorld().hasRaidAt(lv2)) {
            source.sendError(Text.literal("Raid already started close by"));
            return -1;
        }
        RaidManager lv3 = lv.getEntityWorld().getRaidManager();
    ...
    private static int executeStop(ServerCommandSource source) throws CommandSyntaxException {
        ServerPlayerEntity lv = source.getPlayerOrThrow();
        BlockPos lv2 = lv.getBlockPos();
        Raid lv3 = lv.getEntityWorld().getRaidAt(lv2);
        if (lv3 != null) {
            lv3.invalidate();
