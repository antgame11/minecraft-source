...
public class ExecuteCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess) {
        LiteralCommandNode<ServerCommandSource> literalCommandNode = dispatcher.register((LiteralArgumentBuilder)CommandManager.literal("execute").requires(CommandManager.requirePermissionLevel(2)));
    ...
    private static OptionalInt testBlocksCondition(ServerWorld world, BlockPos start, BlockPos end, BlockPos destination, boolean masked) throws CommandSyntaxException {
        ...
        try (ErrorReporter.Logging lv5 = new ErrorReporter.Logging(LOGGER);){
            for (int k = lv.getMinZ(); k <= lv.getMaxZ(); ++k) {
                for (int l = lv.getMinY(); l <= lv.getMaxY(); ++l) {
                    for (int m = lv.getMinX(); m <= lv.getMaxX(); ++m) {
                        BlockPos lv6 = new BlockPos(m, l, k);
                        BlockPos lv7 = lv6.add(lv3);
                        BlockState lv8 = world.getBlockState(lv6);
                        if (masked && lv8.isOf(Blocks.AIR)) continue;
                        if (lv8 != world.getBlockState(lv7)) {
                            OptionalInt optionalInt = OptionalInt.empty();
                            return optionalInt;
                        }
                        BlockEntity lv9 = world.getBlockEntity(lv6);
                        BlockEntity lv10 = world.getBlockEntity(lv7);
                        if (lv9 != null) {
                            ...
                            if (lv10.getType() != lv9.getType()) {
                                optionalInt = OptionalInt.empty();
                                return optionalInt;
                            }
                            if (!lv9.getComponents().equals(lv10.getComponents())) {
                                optionalInt = OptionalInt.empty();
                                return optionalInt;
                            }
                            NbtWriteView lv11 = NbtWriteView.create(lv5.makeChild(lv9.getReporterContext()), lv4);
                            lv9.writeComponentlessData(lv11);
                            NbtCompound lv12 = lv11.getNbt();
                            NbtWriteView lv13 = NbtWriteView.create(lv5.makeChild(lv10.getReporterContext()), lv4);
                            lv10.writeComponentlessData(lv13);
                            NbtCompound lv14 = lv13.getNbt();
                            if (!lv12.equals(lv14)) {
