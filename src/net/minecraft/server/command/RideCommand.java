...
public class RideCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("ride").requires(CommandManager.requirePermissionLevel(2))).then(((RequiredArgumentBuilder)CommandManager.argument("target", EntityArgumentType.entity()).then((ArgumentBuilder<ServerCommandSource, ?>)CommandManager.literal("mount").then((ArgumentBuilder<ServerCommandSource, ?>)CommandManager.argument("vehicle", EntityArgumentType.entity()).executes(context -> RideCommand.executeMount((ServerCommandSource)context.getSource(), EntityArgumentType.getEntity(context, "target"), EntityArgumentType.getEntity(context, "vehicle")))))).then(CommandManager.literal("dismount").executes(context -> RideCommand.executeDismount((ServerCommandSource)context.getSource(), EntityArgumentType.getEntity(context, "target"))))));
    }
