...
public class PlaceCommand {
    ...
    public static int executePlaceTemplate(ServerCommandSource source, Identifier id, BlockPos pos, BlockRotation rotation, BlockMirror mirror, float integrity, int seed, boolean strict) throws CommandSyntaxException {
        ...
        StructurePlacementData lv5 = new StructurePlacementData().setMirror(mirror).setRotation(rotation).setUpdateNeighbors(strict);
        if (integrity < 1.0f) {
        ...
        if (!(bl2 = lv4.place(lv, pos, pos, lv5, StructureBlockBlockEntity.createRandom(seed), Block.NOTIFY_LISTENERS | (strict ? Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS : 0)))) {
