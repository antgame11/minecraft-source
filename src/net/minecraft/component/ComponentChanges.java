...
public final class ComponentChanges {
    ...
    public static final PacketCodec<RegistryByteBuf, ComponentChanges> PACKET_CODEC = ComponentChanges.createPacketCodec(new PacketCodecFunction(){
        ...
        public <T> PacketCodec<RegistryByteBuf, T> apply(ComponentType<T> arg) {
            return arg.getPacketCodec().cast();
        }
    });
    public static final PacketCodec<RegistryByteBuf, ComponentChanges> LENGTH_PREPENDED_PACKET_CODEC = ComponentChanges.createPacketCodec(new PacketCodecFunction(){
        ...
        public <T> PacketCodec<RegistryByteBuf, T> apply(ComponentType<T> arg) {
            PacketCodec lv = arg.getPacketCodec().cast();
            return lv.collect(PacketCodecs.lengthPrependedRegistry(Integer.MAX_VALUE));
        }
    });
        ...
        return new PacketCodec<RegistryByteBuf, ComponentChanges>(){
            ...
            public ComponentChanges decode(RegistryByteBuf arg) {
                ...
                for (l = 0; l < i; ++l) {
                    ...
                    Object object = packetCodecFunction.apply(lv).decode(arg);
            ...
            public void encode(RegistryByteBuf arg, ComponentChanges arg2) {
                ...
                for (Reference2ObjectMap.Entry entry : Reference2ObjectMaps.fastIterable(arg2.changedComponents)) {
                    ...
                    this.encode(arg, lv, optional.get());
                }
                for (Reference2ObjectMap.Entry entry : Reference2ObjectMaps.fastIterable(arg2.changedComponents)) {
