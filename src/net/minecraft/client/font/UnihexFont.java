...
implements Font {
        ...
        public BakedGlyph bake(Glyph.AbstractGlyphBaker baker) {
            return baker.bake(this.getMetrics(), new UploadableGlyph(){
                ...
                public void upload(int x, int y, GpuTexture texture) {
                    ...
                    intBuffer.rewind();
                    RenderSystem.getDevice().createCommandEncoder().writeToTexture(texture, MemoryUtil.memByteBuffer(intBuffer), NativeImage.Format.RGBA, 0, 0, x, y, this.width(), 16);
    ...
    implements FontLoader {
        public static final MapCodec<Loader> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)Identifier.CODEC.fieldOf("hex_file")).forGetter(loader -> loader.sizes), DimensionOverride.CODEC.listOf().optionalFieldOf("size_overrides", List.of()).forGetter(loader -> loader.overrides)).apply((Applicative<Loader, ?>)instance, Loader::new));
