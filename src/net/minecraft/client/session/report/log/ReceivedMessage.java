...
extends ChatLogEntry {
    ...
    {
        public static final MapCodec<ChatMessage> CHAT_MESSAGE_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)Codecs.GAME_PROFILE_CODEC.fieldOf("profile")).forGetter(ChatMessage::profile), SignedMessage.CODEC.forGetter(ChatMessage::message), MessageTrustStatus.CODEC.optionalFieldOf("trust_level", MessageTrustStatus.SECURE).forGetter(ChatMessage::trustStatus)).apply((Applicative<ChatMessage, ?>)instance, ChatMessage::new));
        ...
        public Text getNarration() {
            ...
            return Text.translatable("gui.chatSelection.message.narrate", this.profile.name(), lv, lv2);
        }

        public Text getHeadingText() {
            Text lv = this.getFormattedTimestamp();
            return Text.translatable("gui.chatSelection.heading", this.profile.name(), lv);
        }
