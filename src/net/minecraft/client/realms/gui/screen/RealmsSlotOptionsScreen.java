...
extends RealmsScreen {
    ...
    public void init() {
        ...
        if (this.worldType != RealmsServer.WorldType.NORMAL) {
            ...
            this.addLabel(new RealmsLabel(lv, this.width / 2, 26, -65536));
        }
        ...
        CyclingButtonWidget<Difficulty> lv2 = this.addDrawableChild(CyclingButtonWidget.builder(Difficulty::getTranslatableName).values((Collection<Difficulty>)DIFFICULTIES).initially(this.difficulty).build(i, RealmsSlotOptionsScreen.row(1), this.column2_x, 20, Text.translatable("options.difficulty"), (arg, arg2) -> {
            this.difficulty = arg2;
        }));
        CyclingButtonWidget<GameMode> lv3 = this.addDrawableChild(CyclingButtonWidget.builder(GameMode::getSimpleTranslatableName).values((Collection<GameMode>)GAME_MODES).initially(this.gameMode).build(this.column1_x, RealmsSlotOptionsScreen.row(3), this.column2_x, 20, Text.translatable("selectWorld.gameMode"), (button, gameModeIndex) -> {
            this.gameMode = gameModeIndex;
        }));
        CyclingButtonWidget<Boolean> lv4 = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.forceGameMode).build(i, RealmsSlotOptionsScreen.row(3), this.column2_x, 20, Text.translatable("mco.configure.world.forceGameMode"), (button, forceGameMode) -> {
