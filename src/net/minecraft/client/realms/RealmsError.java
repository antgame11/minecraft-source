...
public interface RealmsError {
    ...
    public record SimpleHttpError(int httpCode, @Nullable Text payload) implements RealmsError
    {
        ...
        public static SimpleHttpError unreadableHtmlBody(int statusCode, String html) {
            int j = html.indexOf(BODY_HTML_OPENING_TAG);
            int k = html.indexOf(BODY_HTML_CLOSING_TAG);
            if (j >= 0 && k > j) {
                return new SimpleHttpError(statusCode, Text.literal(html.substring(j + BODY_HTML_OPENING_TAG.length(), k).trim()));
            }
