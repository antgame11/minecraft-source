...
implements TutorialStepHandler {
    ...
    public void tick() {
        ...
        if (this.ticks == 1 && (lv2 = lv.player) != null) {
            if (lv2.getInventory().contains(ItemTags.LOGS)) {
                this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                return;
            }
            if (FindTreeTutorialStepHandler.hasBrokenTreeBlocks(lv2)) {
                this.manager.setStep(TutorialStep.CRAFT_PLANKS);
                return;
            }
        }
        if ((this.ticks >= 600 || this.punches > 3) && this.toast == null) {
            this.toast = new TutorialToast(lv.textRenderer, TutorialToast.Type.TREE, TITLE, DESCRIPTION, true);
