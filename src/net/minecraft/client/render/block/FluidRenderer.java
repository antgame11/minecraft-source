...
public class FluidRenderer {
    ...
    protected void onResourceReload(BlockModels arg, SpriteHolder arg2) {
        this.lavaSprites[0] = arg.getModel(Blocks.LAVA.getDefaultState()).particleSprite();
        this.lavaSprites[1] = arg2.getSprite(ModelBaker.LAVA_FLOW);
        this.waterSprites[0] = arg.getModel(Blocks.WATER.getDefaultState()).particleSprite();
        this.waterSprites[1] = arg2.getSprite(ModelBaker.WATER_FLOW);
        this.waterOverlaySprite = arg2.getSprite(ModelBaker.WATER_OVERLAY);
    }
    ...
    public void render(BlockRenderView world, BlockPos pos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
        ...
        boolean bl3 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.DOWN, lv2) && !FluidRenderer.shouldSkipRendering(Direction.DOWN, 0.8888889f, lv);
        ...
        if (bl2 && !FluidRenderer.shouldSkipRendering(Direction.UP, Math.min(Math.min(p, r), Math.min(q, o)), lv3)) {
            float ag;
            float af;
            float y;
            p -= 0.001f;
        ...
        for (Direction lv16 : Direction.Type.HORIZONTAL) {
            ...
            }) || FluidRenderer.shouldSkipRendering(lv16, Math.max(ad, y), world.getBlockState(pos.offset(lv16)))) continue;
