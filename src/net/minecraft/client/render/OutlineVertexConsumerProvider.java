...
implements VertexConsumerProvider {
    ...
    public VertexConsumer getBuffer(RenderLayer layer) {
        if (layer.isOutline()) {
            VertexConsumer lv = this.plainDrawer.getBuffer(layer);
            return new OutlineVertexConsumer(lv, this.OUTLINE_COLOR);
        }
        Optional<RenderLayer> optional = layer.getAffectedOutline();
        if (optional.isPresent()) {
            VertexConsumer lv2 = this.plainDrawer.getBuffer(optional.get());
            return new OutlineVertexConsumer(lv2, this.OUTLINE_COLOR);
        }
