...
public class PlayerListHud {
    ...
    public void setVisible(boolean visible) {
        if (this.visible != visible) {
            ...
            if (visible) {
                ...
                this.client.getNarratorManager().narrateSystemImmediately(Text.translatable("multiplayer.player.list.narration", lv));
            }
        }
    }
    ...
    public void render(DrawContext context, int scaledWindowWidth, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
        ...
        for (int x = 0; x < o; ++x) {
            ...
            if (bl) {
                PlayerEntity lv11 = this.client.world.getPlayerByUuid(gameProfile.id());
                boolean bl22 = lv11 != null && PlayerEntityRenderer.shouldFlipUpsideDown(lv11);
                PlayerSkinDrawer.draw(context, lv9.getSkinTextures().body().texturePath(), z, aa, 8, lv9.shouldShowHat(), bl22, -1);
                z += 9;
            }
            ...
            if (objective != null && lv9.getGameMode() != GameMode.SPECTATOR && (ac = (ab = z + k + 1) + r) - ab > 5) {
                this.renderScoreboardObjective(objective, aa, lv10, ab, ac, gameProfile.id(), context);
            }
    ...
    protected void renderLatencyIcon(DrawContext context, int width, int x, int y, PlayerListEntry entry) {
        ...
        context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, lv, x + width - 11, y, 10, 8);
    }
    ...
    private void renderHearts(int y, int left, int right, UUID uuid, DrawContext context, int score) {
        ...
        int m = MathHelper.ceilDiv(Math.max(score, lv.getLastScore()), 2);
        int n = Math.max(score, Math.max(lv.getLastScore(), 20)) / 2;
        ...
        if (o <= 3) {
            ...
            context.drawTextWithShadow(this.client.textRenderer, lv3, (right + left - this.client.textRenderer.getWidth(lv3)) / 2, y, ColorHelper.fullAlpha(p2));
            return;
        }
        Identifier lv4 = bl ? CONTAINER_HEART_BLINKING_TEXTURE : CONTAINER_HEART_TEXTURE;
        for (p = m; p < n; ++p) {
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, lv4, left + p * o, y, 9, 9);
        }
        for (p = 0; p < m; ++p) {
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, lv4, left + p * o, y, 9, 9);
            if (bl) {
                if (p * 2 + 1 < lv.getLastScore()) {
                    context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, FULL_HEART_BLINKING_TEXTURE, left + p * o, y, 9, 9);
                }
                if (p * 2 + 1 == lv.getLastScore()) {
                    context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, HALF_HEART_BLINKING_TEXTURE, left + p * o, y, 9, 9);
                }
            }
            if (p * 2 + 1 < score) {
                context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, p >= 10 ? ABSORBING_FULL_HEART_BLINKING_TEXTURE : FULL_HEART_TEXTURE, left + p * o, y, 9, 9);
            }
            if (p * 2 + 1 != score) continue;
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, p >= 10 ? ABSORBING_HALF_HEART_BLINKING_TEXTURE : HALF_HEART_TEXTURE, left + p * o, y, 9, 9);
        }
    }
