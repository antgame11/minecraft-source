...
extends Screen {
    ...
    public static OptimizeWorldScreen create(MinecraftClient client, BooleanConsumer callback, DataFixer dataFixer, LevelStorage.Session storageSession, boolean eraseCache) {
        ...
        try {
            ...
            OptimizeWorldScreen optimizeWorldScreen = new OptimizeWorldScreen(callback, dataFixer, storageSession, lv4, eraseCache, lv5);
            if (lv3 != null) {
                lv3.close();
            }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        int n = m + 10;
        context.drawCenteredTextWithShadow(this.textRenderer, this.updater.getStatus(), this.width / 2, m - this.textRenderer.fontHeight - 2, Colors.LIGHT_GRAY);
        if (this.updater.getTotalChunkCount() > 0) {
            context.fill(k - 1, m - 1, l + 1, n + 1, Colors.BLACK);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.converted", this.updater.getUpgradedChunkCount()), k, 40, Colors.LIGHT_GRAY);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.skipped", this.updater.getSkippedChunkCount()), k, 40 + this.textRenderer.fontHeight + 3, Colors.LIGHT_GRAY);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.total", this.updater.getTotalChunkCount()), k, 40 + (this.textRenderer.fontHeight + 3) * 2, Colors.LIGHT_GRAY);
            int o = 0;
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, lv2, this.width / 2, m + 2 * this.textRenderer.fontHeight + 2, Colors.LIGHT_GRAY);
            context.drawCenteredTextWithShadow(this.textRenderer, lv3, this.width / 2, m + (n - m) / 2 - this.textRenderer.fontHeight / 2, Colors.LIGHT_GRAY);
        }
    }
}
