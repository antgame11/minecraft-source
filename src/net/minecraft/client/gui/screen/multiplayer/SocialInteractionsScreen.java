...
extends Screen {
    ...
    private void setCurrentTab(Tab currentTab) {
        ...
        if (!this.searchBox.getText().isEmpty() && this.playerList.isEmpty() && !this.searchBox.isFocused()) {
            lv2.narrateSystemImmediately(EMPTY_SEARCH_TEXT);
        } else if (bl) {
            if (currentTab == Tab.HIDDEN) {
                lv2.narrateSystemImmediately(EMPTY_HIDDEN_TEXT);
            } else if (currentTab == Tab.BLOCKED) {
                lv2.narrateSystemImmediately(EMPTY_BLOCKED_TEXT);
            }
        }
    }
    ...
    public void renderBackground(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND_TEXTURE, k, 64, 236, this.getScreenHeight() + 16);
        context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SEARCH_ICON_TEXTURE, k + 10, 76, 12, 12);
    }
