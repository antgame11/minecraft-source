...
public class MipmapHelper {
    ...
    public static NativeImage[] getMipmapLevelsImages(NativeImage[] originals, int mipmap) {
        ...
        for (int j = 1; j <= mipmap; ++j) {
            ...
            for (int m = 0; m < k; ++m) {
                for (int n = 0; n < l; ++n) {
                    lv2.setColorArgb(m, n, MipmapHelper.blend(lv.getColorArgb(m * 2 + 0, n * 2 + 0), lv.getColorArgb(m * 2 + 1, n * 2 + 0), lv.getColorArgb(m * 2 + 0, n * 2 + 1), lv.getColorArgb(m * 2 + 1, n * 2 + 1), bl));
                }
            }
            lvs[j] = lv2;
        }
    ...
    private static int blend(int one, int two, int three, int four, boolean checkAlpha) {
        ...
        return ColorHelper.getArgb(r, s, t, u);
    }
