...
public record UnstitchAtlasSource(Identifier resource, List<Region> regions, double divisorX, double divisorY) implements AtlasSource
{
    ...
    public static final MapCodec<UnstitchAtlasSource> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)Identifier.CODEC.fieldOf("resource")).forGetter(UnstitchAtlasSource::resource), ((MapCodec)Codecs.nonEmptyList(Region.CODEC.listOf()).fieldOf("regions")).forGetter(UnstitchAtlasSource::regions), Codec.DOUBLE.optionalFieldOf("divisor_x", 1.0).forGetter(UnstitchAtlasSource::divisorX), Codec.DOUBLE.optionalFieldOf("divisor_y", 1.0).forGetter(UnstitchAtlasSource::divisorY)).apply((Applicative<UnstitchAtlasSource, ?>)instance, UnstitchAtlasSource::new));
    ...
    implements AtlasSource.SpriteRegion {
        ...
        public SpriteContents apply(SpriteOpener arg) {
            try {
                ...
                SpriteContents spriteContents = new SpriteContents(this.region.sprite, new SpriteDimensions(k, l), lv2);
