...
public final class SaplingGenerator {
    ...
    public boolean generate(ServerWorld world, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, Random random) {
        ...
        if (lv != null && (lv2 = (RegistryEntry)world.getRegistryManager().getOrThrow(RegistryKeys.CONFIGURED_FEATURE).getOptional(lv).orElse(null)) != null) {
            for (int i = 0; i >= -1; --i) {
                for (int j = 0; j >= -1; --j) {
                    ...
                    world.setBlockState(pos.add(i, 0, j), lv4, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
                    world.setBlockState(pos.add(i + 1, 0, j), lv4, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
                    world.setBlockState(pos.add(i, 0, j + 1), lv4, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
                    world.setBlockState(pos.add(i + 1, 0, j + 1), lv4, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
        ...
        world.setBlockState(pos, lv8, Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
