...
extends BlockEntity {
    ...
    public void activate(Direction direction) {
        ...
        this.world.addSyncedBlockEvent(lv, this.getCachedState().getBlock(), 1, direction.getIndex());
    }
    ...
    private void notifyMemoriesOfBell() {
        ...
        if (!this.world.isClient()) {
            for (LivingEntity lv3 : this.hearingEntities) {
                if (!lv3.isAlive() || lv3.isRemoved() || !lv.isWithinDistance(lv3.getEntityPos(), 32.0)) continue;
    ...
    private static boolean raidersHearBell(BlockPos pos, List<LivingEntity> hearingEntities) {
        for (LivingEntity lv : hearingEntities) {
            if (!lv.isAlive() || lv.isRemoved() || !pos.isWithinDistance(lv.getEntityPos(), 32.0) || !lv.getType().isIn(EntityTypeTags.RAIDERS)) continue;
            return true;
        }
        return false;
    }
