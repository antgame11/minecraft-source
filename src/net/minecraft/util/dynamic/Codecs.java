...
public class Codecs {
    ...
    public static final Codec<Vector2f> VECTOR_2F = Codec.FLOAT.listOf().comapFlatMap(list -> Util.decodeFixedLengthList(list, 2).map(listx -> new Vector2f(((Float)listx.get(0)).floatValue(), ((Float)listx.get(1)).floatValue())), vec2f -> List.of(Float.valueOf(vec2f.x()), Float.valueOf(vec2f.y())));
    ...
    public static final Codec<Vector3i> VECTOR_3I = Codec.INT.listOf().comapFlatMap(list -> Util.decodeFixedLengthList(list, 3).map(listx -> new Vector3i((Integer)listx.get(0), (Integer)listx.get(1), (int)((Integer)listx.get(2)))), vec3i -> List.of(Integer.valueOf(vec3i.x()), Integer.valueOf(vec3i.y()), Integer.valueOf(vec3i.z())));
    ...
    public static final Codec<PropertyMap> GAME_PROFILE_PROPERTY_MAP = Codec.either(Codec.unboundedMap(Codec.STRING, Codec.STRING.listOf()), GAME_PROFILE_PROPERTY.listOf()).xmap(either -> {
        ...
        either.ifLeft(map -> map.forEach((key, values) -> {
            for (String string2 : values) {
                builder.put(key, new Property((String)key, string2));
            }
        })).ifRight(properties -> {
    ...
    private static MapCodec<GameProfile> createGameProfileCodec(Codec<UUID> uuidCodec) {
        return RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)uuidCodec.fieldOf("id")).forGetter(GameProfile::id), ((MapCodec)PLAYER_NAME.fieldOf("name")).forGetter(GameProfile::name), GAME_PROFILE_PROPERTY_MAP.optionalFieldOf("properties", PropertyMap.EMPTY).forGetter(GameProfile::properties)).apply((Applicative<GameProfile, ?>)instance, GameProfile::new));
    }
    ...
    public static <E extends Enum<E>> Codec<E> enumByName(Function<String, E> valueOf) {
        return Codec.STRING.comapFlatMap(id -> {
            try {
                return DataResult.success((Enum)valueOf.apply((String)id));
            } catch (IllegalArgumentException illegalArgumentException) {
