...
public class MathHelper {
    ...
    public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
        ...
        int m = MathHelper.clamp(seed, lowerBound, upperBound);
        return IntStream.iterate(m, i2 -> {
            int m = java.lang.Math.abs(m - i2);
            return m - m >= lowerBound || m + m <= upperBound;
        }, i2 -> {
            int o;
            boolean bl2;
            boolean bl = i2 <= m;
            int n = java.lang.Math.abs(m - i2);
            boolean bl3 = bl2 = m + n + steps <= upperBound;
            if (!(bl && bl2 || (o = m - n - (bl ? steps : 0)) < lowerBound)) {
                return o;
            }
            return m + n + steps;
        });
