...
public class MultiNoiseBiomeSourceParameterList {
    ...
    public record Preset(Identifier id, BiomeSourceFunction biomeSourceFunction) {
        ...
        public static final Codec<Preset> CODEC = Identifier.CODEC.flatXmap(id -> Optional.ofNullable(BY_IDENTIFIER.get(id)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown preset: " + String.valueOf(id))), preset -> DataResult.success(preset.id));
