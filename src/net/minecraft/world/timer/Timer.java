...
public class Timer<T> {
    ...
    private void addEvent(NbtCompound nbt) {
        TimerCallback lv = nbt.get(CALLBACK_KEY, this.callback.getCodec()).orElse(null);
        if (lv != null) {
            String string = nbt.getString(NAME_KEY, "");
            long l = nbt.getLong(TRIGGER_TIME_KEY, 0L);
            this.setEvent(string, l, lv);
        }
    }
    ...
    private NbtCompound serialize(Event<T> event) {
        ...
        lv.put(CALLBACK_KEY, this.callback.getCodec(), event.callback);
        return lv;
    }
