...
implements Comparable<LevelSummary> {
    ...
    private Text createDetails() {
        ...
        MutableText mutableText = lv = this.isHardcore() ? Text.empty().append(Text.translatable("gameMode.hardcore").withColor(Colors.RED)) : Text.translatable("gameMode." + this.getGameMode().getId());
        if (this.hasCheats()) {
