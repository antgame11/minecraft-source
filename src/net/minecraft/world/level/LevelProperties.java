...
SaveProperties {
    ...
    private void updateProperties(DynamicRegistryManager registryManager, NbtCompound levelNbt, @Nullable NbtCompound playerNbt) {
        ...
        NbtCompound lv = new NbtCompound();
        lv.putString("Name", SharedConstants.getGameVersion().name());
        lv.putInt("Id", SharedConstants.getGameVersion().dataVersion().id());
        lv.putBoolean("Snapshot", !SharedConstants.getGameVersion().stable());
        lv.putString("Series", SharedConstants.getGameVersion().dataVersion().series());
        ...
        WorldGenSettings.encode(dynamicOps, this.generatorOptions, registryManager).resultOrPartial(Util.addPrefix("WorldGenSettings: ", LOGGER::error)).ifPresent(worldGenSettings -> levelNbt.put(WORLD_GEN_SETTINGS_KEY, (NbtElement)worldGenSettings));
