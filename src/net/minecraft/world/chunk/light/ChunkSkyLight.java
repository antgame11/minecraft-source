...
public class ChunkSkyLight {
    ...
    private int calculateSurfaceY(Chunk chunk, int topSectionIndex, int localX, int localZ) {
        ...
        for (int m = topSectionIndex; m >= 0; --m) {
            ...
            for (n = 15; n >= 0; --n) {
                ...
                if (ChunkSkyLight.faceBlocksLight(lv3, lv5)) {
                    return lv.getY();
                }
                lv3 = lv5;
                lv.set(lv2);
    ...
    private int locateLightBlockingBlockBelow(BlockView blockView, BlockPos pos, BlockState blockState) {
        ...
        while (lv2.getY() >= this.minY) {
            ...
            if (ChunkSkyLight.faceBlocksLight(lv3, lv4)) {
                return lv.getY();
            }
            lv3 = lv4;
            lv.set(lv2);
    ...
    private static boolean faceBlocksLight(BlockState upper, BlockState lower) {
        ...
        VoxelShape lv = ChunkLightProvider.getOpaqueShape(upper, Direction.DOWN);
        VoxelShape lv2 = ChunkLightProvider.getOpaqueShape(lower, Direction.UP);
