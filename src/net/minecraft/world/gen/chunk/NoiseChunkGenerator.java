...
extends ChunkGenerator {
    ...
    public void appendDebugHudText(List<String> text, NoiseConfig noiseConfig, BlockPos pos) {
        ...
        text.add("NoiseRouter T: " + decimalFormat.format(lv.temperature().sample(lv2)) + " V: " + decimalFormat.format(lv.vegetation().sample(lv2)) + " C: " + decimalFormat.format(lv.continents().sample(lv2)) + " E: " + decimalFormat.format(lv.erosion().sample(lv2)) + " D: " + decimalFormat.format(lv.depth().sample(lv2)) + " W: " + decimalFormat.format(d) + " PV: " + decimalFormat.format(DensityFunctions.getPeaksValleysNoise((float)d)) + " PS: " + decimalFormat.format(lv.preliminarySurfaceLevel().sample(lv2)) + " N: " + decimalFormat.format(lv.finalDensity().sample(lv2)));
    }
    ...
    private BlockState getBlockState(ChunkNoiseSampler chunkNoiseSampler, int x, int y, int z, BlockState state) {
        int l;
        int m;
        if (SharedConstants.AQUIFERS && z >= 0 && z % 4 == 0 && y == (m = (l = chunkNoiseSampler.estimateSurfaceHeight(x, z)) + 8)) {
            state = m < this.getSeaLevel() ? Blocks.SLIME_BLOCK.getDefaultState() : Blocks.HONEY_BLOCK.getDefaultState();
        }
        return state;
    }
