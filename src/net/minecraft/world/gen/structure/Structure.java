...
public abstract class Structure {
    ...
    public StructureStart createStructureStart(RegistryEntry<Structure> structure, RegistryKey<World> dimension, DynamicRegistryManager dynamicRegistryManager, ChunkGenerator chunkGenerator, BiomeSource biomeSource, NoiseConfig noiseConfig, StructureTemplateManager structureTemplateManager, long seed, ChunkPos chunkPos, int references, HeightLimitView world, Predicate<RegistryEntry<Biome>> validBiomes) {
        StructurePiecesCollector lv3;
        StructureStart lv4;
        Finishable lv = FlightProfiler.INSTANCE.startStructureGenerationProfiling(chunkPos, dimension, structure);
        Context lv2 = new Context(dynamicRegistryManager, chunkGenerator, biomeSource, noiseConfig, structureTemplateManager, seed, chunkPos, world, validBiomes);
        Optional<StructurePosition> optional = this.getValidStructurePosition(lv2);
        if (optional.isPresent() && (lv4 = new StructureStart(this, chunkPos, references, (lv3 = optional.get().generate()).toList())).hasChildren()) {
            if (lv != null) {
                lv.finish(true);
            }
            return lv4;
