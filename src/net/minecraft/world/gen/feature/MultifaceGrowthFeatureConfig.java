...
implements FeatureConfig {
    public static final Codec<MultifaceGrowthFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Registries.BLOCK.getCodec().fieldOf("block")).flatXmap(MultifaceGrowthFeatureConfig::validateBlock, DataResult::success).orElse((MultifaceGrowthBlock)Blocks.GLOW_LICHEN).forGetter(config -> config.block), ((MapCodec)Codec.intRange(1, 64).fieldOf("search_range")).orElse(10).forGetter(config -> config.searchRange), ((MapCodec)Codec.BOOL.fieldOf("can_place_on_floor")).orElse(false).forGetter(config -> config.placeOnFloor), ((MapCodec)Codec.BOOL.fieldOf("can_place_on_ceiling")).orElse(false).forGetter(config -> config.placeOnCeiling), ((MapCodec)Codec.BOOL.fieldOf("can_place_on_wall")).orElse(false).forGetter(config -> config.placeOnWalls), ((MapCodec)Codec.floatRange(0.0f, 1.0f).fieldOf("chance_of_spreading")).orElse(Float.valueOf(0.5f)).forGetter(config -> Float.valueOf(config.spreadChance)), ((MapCodec)RegistryCodecs.entryList(RegistryKeys.BLOCK).fieldOf("can_be_placed_on")).forGetter(config -> config.canPlaceOn)).apply((Applicative<MultifaceGrowthFeatureConfig, ?>)instance, MultifaceGrowthFeatureConfig::new));
