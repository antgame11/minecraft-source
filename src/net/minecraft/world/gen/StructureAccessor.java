...
public class StructureAccessor {
    ...
    public StructureStart getStructureContaining(BlockPos pos, Predicate<RegistryEntry<Structure>> predicate) {
        RegistryWrapper.Impl lv = this.getRegistryManager().getOrThrow(RegistryKeys.STRUCTURE);
        for (StructureStart lv2 : this.getStructureStarts(new ChunkPos(pos), (Structure structure) -> ((Registry)lv).getEntry(((Registry)lv).getRawId(structure)).map(predicate::test).orElse(false))) {
