...
extends AnimalEntity {
    ...
    private void playEatingAnimation() {
        if (this.getEatingTicks() % 5 == 0) {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                this.getEntityWorld().addParticleClient(new ItemStackParticleEffect(ParticleTypes.ITEM, this.getEquippedStack(EquipmentSlot.MAINHAND)), lv2.x, lv2.y, lv2.z, lv.x, lv.y + 0.05, lv.z);
            }
        }
    }
    ...
    private void sneeze() {
        ...
        lv2.addParticleClient(ParticleTypes.SNEEZE, this.getX() - (double)(this.getWidth() + 1.0f) * 0.5 * (double)MathHelper.sin(this.bodyYaw * ((float)Math.PI / 180)), this.getEyeY() - (double)0.1f, this.getZ() + (double)(this.getWidth() + 1.0f) * 0.5 * (double)MathHelper.cos(this.bodyYaw * ((float)Math.PI / 180)), lv.x, 0.0, lv.z);
    ...
    extends Goal {
        ...
        public void stop() {
            ...
            if (!lv.isEmpty()) {
                PandaEntity.this.dropStack(PickUpFoodGoal.castToServerWorld(PandaEntity.this.getEntityWorld()), lv);
