...
implements ItemSteerable {
    ...
    protected Vec3d getPassengerAttachmentPos(Entity passenger, EntityDimensions dimensions, float scaleFactor) {
        ...
        float h = this.limbAnimator.getAnimationProgress();
    ...
    public LivingEntity getControllingPassenger() {
        PlayerEntity lv;
        Entity entity;
        if (this.hasSaddleEquipped() && (entity = this.getFirstPassenger()) instanceof PlayerEntity && (lv = (PlayerEntity)entity).isHolding(Items.WARPED_FUNGUS_ON_A_STICK)) {
            return lv;
        }
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        for (BlockPos lv3 : set) {
            double g;
            if (this.getEntityWorld().getFluidState(lv3).isIn(FluidTags.LAVA) || !Dismounting.canDismountInBlock(g = this.getEntityWorld().getDismountHeight(lv3))) continue;
            Vec3d lv4 = Vec3d.ofCenter(lv3, g);
            for (EntityPose lv5 : passenger.getPoses()) {
                Box lv6 = passenger.getBoundingBox(lv5);
                if (!Dismounting.canPlaceEntityAt(this.getEntityWorld(), passenger, lv6.offset(lv4))) continue;
    ...
    private void updateFloating() {
        if (this.isInLava()) {
            ...
            if (!lv.isAbove(FluidBlock.COLLISION_SHAPE, this.getBlockPos(), true) || this.getEntityWorld().getFluidState(this.getBlockPos().up()).isIn(FluidTags.LAVA)) {
