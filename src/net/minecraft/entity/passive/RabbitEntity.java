...
extends AnimalEntity {
    ...
    public void mobTick(ServerWorld world) {
        ...
        if (this.isOnGround()) {
            ...
            if (this.getVariant() == Variant.EVIL && this.ticksUntilJump == 0 && (lv = this.getTarget()) != null && this.squaredDistanceTo(lv) < 16.0) {
    ...
    public RabbitEntity createChild(ServerWorld arg, PassiveEntity arg2) {
        block2: {
            block3: {
                ...
                lv2 = RabbitEntity.getVariantFromPos(arg, this.getBlockPos());
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        Variant lv = RabbitEntity.getVariantFromPos(world, this.getBlockPos());
        if (entityData instanceof RabbitData) {
            lv = ((RabbitData)entityData).variant;
        } else {
    ...
    extends MoveToTargetPosGoal {
        ...
        public void tick() {
            ...
            if (this.hasReached()) {
                World lv = this.rabbit.getEntityWorld();
