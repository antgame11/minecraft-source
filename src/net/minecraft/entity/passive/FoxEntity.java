...
extends AnimalEntity {
        ...
        if (!this.getEntityWorld().isClient() && this.isAlive() && this.canActVoluntarily()) {
            ...
            if (this.canEat(lv)) {
                if (this.eatingTime > 600) {
                    ItemStack lv2 = lv.finishUsing(this.getEntityWorld(), this);
                    if (!lv2.isEmpty()) {
                    ...
                    this.playEatSound();
                    this.getEntityWorld().sendEntityStatus(this, EntityStatuses.CREATE_EATING_PARTICLES);
                }
            }
        ...
        if (status == EntityStatuses.CREATE_EATING_PARTICLES) {
            ...
            if (!lv.isEmpty()) {
                for (int i = 0; i < 8; ++i) {
                    ...
                    this.getEntityWorld().addParticleClient(new ItemStackParticleEffect(ParticleTypes.ITEM, lv), this.getX() + this.getRotationVector().x / 2.0, this.getY(), this.getZ() + this.getRotationVector().z / 2.0, lv2.x, lv2.y + 0.05, lv2.z);
                }
            }
        } else {
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        ...
        Variant lv2 = Variant.fromBiome(lv);
        boolean bl = false;
    ...
    private void spit(ItemStack stack) {
        ...
        ItemEntity lv = new ItemEntity(this.getEntityWorld(), this.getX() + this.getRotationVector().x, this.getY() + 1.0, this.getZ() + this.getRotationVector().z, stack);
    ...
    private void dropItem(ItemStack stack) {
        ItemEntity lv = new ItemEntity(this.getEntityWorld(), this.getX(), this.getY(), this.getZ(), stack);
        this.getEntityWorld().spawnEntity(lv);
    }
    ...
    public void tick() {
        ...
        if (this.canActVoluntarily()) {
            ...
            if (this.isWalking() && this.getEntityWorld().random.nextFloat() < 0.2f) {
                ...
                this.getEntityWorld().syncWorldEvent(WorldEvents.BLOCK_BROKEN, lv, Block.getRawIdFromState(lv2));
            }
        }
    ...
    extends DiveJumpingGoal {
        ...
        public void tick() {
            ...
            if (lv != null && FoxEntity.this.distanceTo(lv) <= 2.0f) {
                FoxEntity.this.tryAttack(JumpChasingGoal.castToServerWorld(FoxEntity.this.getEntityWorld()), lv);
    ...
    extends EscapeSunlightGoal {
        ...
        public boolean canStart() {
            ...
            return FoxEntity.this.getEntityWorld().isDay() && FoxEntity.this.getEntityWorld().isSkyVisible(lv) && !((ServerWorld)FoxEntity.this.getEntityWorld()).isNearOccupiedPointOfInterest(lv) && this.targetShadedPos();
        }
    ...
    extends MoveToTargetPosGoal {
        ...
        protected void eatBerries() {
            if (!EatBerriesGoal.castToServerWorld(FoxEntity.this.getEntityWorld()).getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                return;
            }
            BlockState lv = FoxEntity.this.getEntityWorld().getBlockState(this.targetPos);
        ...
        private void pickSweetBerries(BlockState state) {
            ...
            FoxEntity.this.getEntityWorld().setBlockState(this.targetPos, (BlockState)state.with(SweetBerryBushBlock.AGE, 1), Block.NOTIFY_LISTENERS);
    ...
    extends ActiveTargetGoal<LivingEntity> {
        ...
        public boolean canStart() {
            ...
            ServerWorld lv = DefendFriendGoal.castToServerWorld(FoxEntity.this.getEntityWorld());
            for (LazyEntityReference<LivingEntity> lv2 : FoxEntity.this.getTrustedEntities().toList()) {
                LivingEntity lv3 = lv2.getEntityByClass(lv, LivingEntity.class);
    ...
    extends Goal {
        ...
        protected boolean isAtFavoredLocation() {
            ...
            return !FoxEntity.this.getEntityWorld().isSkyVisible(lv) && FoxEntity.this.getPathfindingFavor(lv) >= 0.0f;
        }
    ...
    implements TargetPredicate.EntityPredicate {
        ...
        public boolean test(LivingEntity target, ServerWorld world) {
            ...
            if (target instanceof PlayerEntity && ((lv = (PlayerEntity)target).isSpectator() || lv.isCreative())) {
                return false;
            }
            if (FoxEntity.this.canTrust(target)) {
                return false;
            }
