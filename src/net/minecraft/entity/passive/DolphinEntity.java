...
extends WaterAnimalEntity {
    ...
    public void tick() {
        ...
        if (this.getEntityWorld().isClient() && this.isTouchingWater() && this.getVelocity().lengthSquared() > 0.03) {
            ...
            for (int i = 0; i < 2; ++i) {
                this.getEntityWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - lv.x * (double)h + (double)f, this.getY() - lv.y, this.getZ() - lv.z * (double)h + (double)g, 0.0, 0.0, 0.0);
                this.getEntityWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - lv.x * (double)h - (double)f, this.getY() - lv.y, this.getZ() - lv.z * (double)h - (double)g, 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    protected boolean isNearTarget() {
        ...
        if (lv != null) {
            return lv.isWithinDistance(this.getEntityPos(), 12.0);
        }
        return false;
    }
    ...
    extends Goal {
        ...
        public boolean shouldContinue() {
            BlockPos lv = this.dolphin.treasurePos;
            if (lv == null) {
                return false;
            }
            return !BlockPos.ofFloored(lv.getX(), this.dolphin.getY(), lv.getZ()).isWithinDistance(this.dolphin.getEntityPos(), 4.0) && !this.noPathToStructure && this.dolphin.getAir() >= 100;
        }
        ...
        public void start() {
            ...
            ServerWorld lv = (ServerWorld)this.dolphin.getEntityWorld();
        ...
        public void stop() {
            BlockPos lv = this.dolphin.treasurePos;
            if (lv == null || BlockPos.ofFloored(lv.getX(), this.dolphin.getY(), lv.getZ()).isWithinDistance(this.dolphin.getEntityPos(), 4.0) || this.noPathToStructure) {
        ...
        public void tick() {
            ...
            World lv = this.dolphin.getEntityWorld();
            if (this.dolphin.isNearTarget() || this.dolphin.getNavigation().isIdle()) {
                BlockPos lv4;
                Vec3d lv2 = Vec3d.ofCenter(this.dolphin.treasurePos);
    ...
    extends Goal {
        ...
        private void spitOutItem(ItemStack stack) {
            ...
            ItemEntity lv = new ItemEntity(DolphinEntity.this.getEntityWorld(), DolphinEntity.this.getX(), d, DolphinEntity.this.getZ(), stack);
            ...
            DolphinEntity.this.getEntityWorld().spawnEntity(lv);
        }
    }
}
