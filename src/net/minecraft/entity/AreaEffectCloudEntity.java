    ...
    private void clientTick() {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (lv.getType() == ParticleTypes.ENTITY_EFFECT) {
                ...
                this.getEntityWorld().addImportantParticleClient(lv, d, e, l, 0.0, 0.0, 0.0);
                continue;
            }
            if (bl) {
                this.getEntityWorld().addImportantParticleClient(lv, d, e, l, 0.0, 0.0, 0.0);
            ...
            this.getEntityWorld().addImportantParticleClient(lv, d, e, l, (0.5 - this.random.nextDouble()) * 0.15, 0.01f, (0.5 - this.random.nextDouble()) * 0.15);
        }
    }
    ...
    private void serverTick(ServerWorld world) {
        ...
        if (this.age % 5 == 0) {
            ...
            if (!this.potionContentsComponent.hasEffects()) {
                ...
                if (!list2.isEmpty()) {
                    for (LivingEntity lv : list2) {
                        double e;
                        double d;
                        double g;
                        if (this.affectedEntities.containsKey(lv) || !lv.isAffectedBySplashPotions()) continue;
                        if (list.stream().noneMatch(lv::canHaveStatusEffect) || !((g = (d = lv.getX() - this.getX()) * d + (e = lv.getZ() - this.getZ()) * e) <= (double)(f * f))) continue;
                        this.affectedEntities.put(lv, this.age + this.reapplicationDelay);
                        for (StatusEffectInstance lv2 : list) {
                            if (lv2.getEffectType().value().isInstant()) {
                                lv2.getEffectType().value().applyInstantEffect(world, this, this.getOwner(), lv, lv2.getAmplifier(), 0.5);
                                continue;
                            }
                            lv.addStatusEffect(new StatusEffectInstance(lv2), this);
                        }
