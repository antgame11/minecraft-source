...
public interface EntityCollisionHandler {
    ...
    implements EntityCollisionHandler {
        ...
        private void update() {
            for (CollisionEvent lv : ALL_EVENTS) {
                List<Consumer<Entity>> list = this.preCallbacks.get((Object)lv);
                this.callbacks.addAll(list);
                list.clear();
                if (this.activeEvents.remove((Object)lv)) {
                    this.callbacks.add(lv.getAction());
                }
                List<Consumer<Entity>> list2 = this.postCallbacks.get((Object)lv);
