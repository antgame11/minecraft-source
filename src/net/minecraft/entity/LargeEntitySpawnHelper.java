...
public class LargeEntitySpawnHelper {
    public static <T extends MobEntity> Optional<T> trySpawnAt(EntityType<T> entityType, SpawnReason reason, ServerWorld world, BlockPos pos, int tries, int horizontalRange, int verticalRange, Requirements requirements, boolean requireEmptySpace) {
        ...
        for (int l = 0; l < tries; ++l) {
            ...
            if (!world.getWorldBorder().contains(lv) || !LargeEntitySpawnHelper.findSpawnPos(world, verticalRange, lv, requirements) || requireEmptySpace && !world.isSpaceEmpty(entityType.getSpawnBox((double)lv.getX() + 0.5, lv.getY(), (double)lv.getZ() + 0.5)) || (lv2 = (MobEntity)entityType.create(world, null, lv, reason, false, false)) == null) continue;
