...
extends AbstractSittingPhase {
    ...
    private final TargetPredicate CLOSE_PLAYER_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(20.0).setPredicate((player, world) -> Math.abs(player.getY() - arg.getY()) <= 10.0);
    private int ticks;
    ...
    public void serverTick(ServerWorld world) {
        ++this.ticks;
        PlayerEntity lv = world.getClosestPlayer(this.CLOSE_PLAYER_PREDICATE, this.dragon, this.dragon.getX(), this.dragon.getY(), this.dragon.getZ());
        if (lv != null) {
            if (this.ticks > 25) {
            ...
            lv = world.getClosestPlayer(PLAYER_WITHIN_RANGE_PREDICATE, this.dragon, this.dragon.getX(), this.dragon.getY(), this.dragon.getZ());
