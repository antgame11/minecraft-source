...
extends PathNodeMaker {
    ...
    protected PathNode getPathNode(int x, int y, int z, int maxYStep, double lastFeetY, Direction direction, PathNodeType nodeType) {
        ...
        if (e - lastFeetY > this.getStepHeight()) {
            return null;
        }
        ...
        if ((lv == null || lv.penalty < 0.0f) && maxYStep > 0 && (lv3 != PathNodeType.FENCE || this.canWalkOverFences()) && lv3 != PathNodeType.UNPASSABLE_RAIL && lv3 != PathNodeType.TRAPDOOR && lv3 != PathNodeType.POWDER_SNOW) {
            lv = this.getJumpOnTopNode(x, y, z, maxYStep, lastFeetY, direction, nodeType, lv2);
    ...
    private PathNode getJumpOnTopNode(int x, int y, int z, int maxYStep, double lastFeetY, Direction direction, PathNodeType nodeType, BlockPos.Mutable mutablePos) {
        PathNode lv = this.getPathNode(x, y + 1, z, maxYStep - 1, lastFeetY, direction, nodeType);
        if (lv == null) {
            return null;
        }
    ...
    private PathNode getOpenNode(int x, int y, int z) {
        for (int l = y - 1; l >= this.entity.getEntityWorld().getBottomY(); --l) {
