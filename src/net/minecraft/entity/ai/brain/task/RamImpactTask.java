...
extends MultiTickTask<GoatEntity> {
    ...
    protected void keepRunning(ServerWorld arg, GoatEntity arg2, long l) {
        ...
        if (!list.isEmpty()) {
            ...
            if (lv2.damage(arg, lv3 = arg.getDamageSources().mobAttackNoAggro(arg2), f = (float)arg2.getAttributeValue(EntityAttributes.ATTACK_DAMAGE))) {
            ...
            float h = MathHelper.clamp(arg2.getMovementSpeed() * 1.65f, 0.2f, 3.0f) + g;
            DamageSource lv4 = arg.getDamageSources().mobAttack(arg2);
            float k = lv2.getDamageBlockedAmount(arg, lv4, f);
            float m = k > 0.0f ? 0.5f : 1.0f;
            lv2.takeKnockback((double)(m * h) * this.strengthMultiplierFactory.applyAsDouble(arg2), this.direction.getX(), this.direction.getZ());
    ...
    private boolean shouldSnapHorn(ServerWorld world, GoatEntity goat) {
        ...
        BlockPos lv2 = BlockPos.ofFloored(goat.getEntityPos().add(lv));
