...
extends Goal {
    ...
    public boolean canStart() {
        this.targetEntity = FleeEntityGoal.getServerWorld(this.mob).getClosestEntity(this.mob.getEntityWorld().getEntitiesByClass(this.classToFleeFrom, this.mob.getBoundingBox().expand(this.fleeDistance, 3.0, this.fleeDistance), arg -> true), this.withinRangePredicate, this.mob, this.mob.getX(), this.mob.getY(), this.mob.getZ());
        if (this.targetEntity == null) {
            return false;
        }
        Vec3d lv = NoPenaltyTargeting.findFrom(this.mob, 16, 7, ((Entity)this.targetEntity).getEntityPos());
        if (lv == null) {
            return false;
        }
