...
extends DiveJumpingGoal {
    ...
    private boolean isWater(BlockPos pos, int offsetX, int offsetZ, int multiplier) {
        ...
        return this.dolphin.getEntityWorld().getFluidState(lv).isIn(FluidTags.WATER) && !this.dolphin.getEntityWorld().getBlockState(lv).blocksMovement();
    }
    ...
    public void tick() {
        ...
        if (!bl) {
            FluidState lv = this.dolphin.getEntityWorld().getFluidState(this.dolphin.getBlockPos());
