...
extends Goal {
    ...
    public void tick() {
        ...
        if (!lv.isWithinDistance(this.mob.getEntityPos(), this.getDesiredDistanceToTarget())) {
            this.reached = false;
            ++this.tryingTime;
        ...
        while (k <= j) {
            for (int l = 0; l < i; ++l) {
                ...
                while (m <= l) {
                    ...
                    while (n <= l) {
                        lv2.set(lv, m, k - 1, n);
                        if (this.mob.isInPositionTargetRange(lv2) && this.isTargetPos(this.mob.getEntityWorld(), lv2)) {
                            this.targetPos = lv2;
                            return true;
                        }
                        n = n > 0 ? -n : 1 - n;
