...
public class BreezeBrain {
    ...
    private static void addIdleTasks(Brain<BreezeEntity> brain) {
        brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, UpdateAttackTargetTask.create((world, breeze) -> breeze.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE))), Pair.of(1, UpdateAttackTargetTask.create((world, breeze) -> breeze.getHurtBy())), Pair.of(2, new SlideAroundTask(20, 40)), Pair.of(3, new RandomTask(ImmutableList.of(Pair.of(new WaitTask(20, 100), 1), Pair.of(StrollTask.create(0.6f), 2))))));
    }

    private static void addFightTasks(BreezeEntity breeze, Brain<BreezeEntity> brain) {
        brain.setTaskList(Activity.FIGHT, ImmutableList.of(Pair.of(0, ForgetAttackTargetTask.create(Sensor.hasTargetBeenAttackableRecently(breeze, 100).negate()::test)), Pair.of(1, new BreezeShootTask()), Pair.of(2, new BreezeJumpTask()), Pair.of(3, new BreezeShootIfStuckTask()), Pair.of(4, new BreezeSlideTowardsTargetTask())), ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryModuleState.VALUE_PRESENT), Pair.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT)));
    }
