...
implements Vibrations {
        ...
        if ((float)animationState.getTimeInMilliseconds(this.age) < 4500.0f) {
            ...
            if (lv2.getRenderType() != BlockRenderType.INVISIBLE) {
                for (int i = 0; i < 30; ++i) {
                    ...
                    this.getEntityWorld().addParticleClient(new BlockStateParticleEffect(ParticleTypes.BLOCK, lv2), d, e, f, 0.0, 0.0, 0.0);
                }
            }
        }
    }
    ...
    public boolean isValidTarget(@Nullable Entity entity) {
        ...
        if (!this.getEntityWorld().getWorldBorder().contains(lv.getBoundingBox())) return false;
        return true;
    }
