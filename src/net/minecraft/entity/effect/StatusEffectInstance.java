...
implements Comparable<StatusEffectInstance> {
    ...
    public StatusEffectInstance withScaledDuration(float durationMultiplier) {
        StatusEffectInstance lv = new StatusEffectInstance(this);
        lv.duration = lv.mapDuration(duration -> Math.max(MathHelper.floor((float)duration * durationMultiplier), 1));
        return lv;
    }
    ...
    static class Fading {
        ...
        public void update(StatusEffectInstance effect) {
            ...
            int n = i = bl ? lv.getFadeInTicks() : lv.getFadeOutTicks();
            if (i == 0) {
                this.factor = f;
            } else {
