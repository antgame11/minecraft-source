...
extends DataFix {
    ...
    private Dynamic<?> fixPlayerFields(Dynamic<?> dynamic) {
        ...
        if (optional.isPresent() && optional2.isPresent() && optional3.isPresent()) {
            Dynamic<Object> dynamic2 = dynamic.createMap(Map.of(dynamic.createString("pos"), FixUtil.createBlockPos(dynamic, optional.get().intValue(), optional2.get().intValue(), optional3.get().intValue())));
        ...
        if ((optional4 = dynamic.get("enteredNetherPosition").result()).isPresent()) {
            dynamic = dynamic.remove("enteredNetherPosition").set("entered_nether_pos", dynamic.createList(Stream.of(dynamic.createDouble(optional4.get().get("x").asDouble(0.0)), dynamic.createDouble(optional4.get().get("y").asDouble(0.0)), dynamic.createDouble(optional4.get().get("z").asDouble(0.0)))));
        }
        return dynamic;
    }
