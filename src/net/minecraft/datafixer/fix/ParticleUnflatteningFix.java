...
extends DataFix {
    ...
    private <T> Dynamic<T> fixItemParticle(Dynamic<T> dynamic, String params) {
        ...
        if (i == -1) {
            ...
            Dynamic dynamic3 = ParticleUnflatteningFix.tryParse(dynamic.getOps(), params.substring(i));
            if (dynamic3 != null) {
    ...
    private static <T> Dynamic<T> tryParse(DynamicOps<T> dynamicOps, String string) {
        try {
            return new Dynamic<T>(dynamicOps, StringNbtReader.fromOps(dynamicOps).read(string));
