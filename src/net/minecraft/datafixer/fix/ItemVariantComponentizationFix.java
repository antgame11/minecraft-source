...
extends DataFix {
    ...
    private static <T> Dynamic<T> fixTropicalFishBucket(Dynamic<T> dynamic2, Dynamic<T> dynamic22) {
        Optional<Number> optional = dynamic22.get("BucketVariantTag").asNumber().result();
        if (optional.isEmpty()) {
            return dynamic2;
        }
    ...
    private static <T> Dynamic<T> fixAxolotlBucket(Dynamic<T> dynamic2, Dynamic<T> dynamic22) {
        Optional<Number> optional = dynamic22.get("Variant").asNumber().result();
        if (optional.isEmpty()) {
            return dynamic2;
        }
    ...
    private static <T> Dynamic<T> fixSalmonBucket(Dynamic<T> dynamic2, Dynamic<T> dynamic22) {
        Optional<Dynamic<T>> optional = dynamic22.get("type").result();
        if (optional.isEmpty()) {
            return dynamic2;
        }
