...
extends ChoiceWriteReadFix {
    ...
    private static <T> Dynamic<T> updateFront(Dynamic<T> signData) {
        ...
        Dynamic dynamic3 = signData.emptyMap().set("messages", signData.createList(list.stream())).set("color", signData.get("Color").result().orElse(signData.createString(DEFAULT_COLOR))).set("has_glowing_text", signData.get("GlowingText").result().orElse(signData.createBoolean(false)));
