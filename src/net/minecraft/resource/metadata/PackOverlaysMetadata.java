...
public record PackOverlaysMetadata(List<Entry> overlays) {
    ...
    public static Codec<PackOverlaysMetadata> createCodec(ResourceType type) {
        return RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Entry.createCodec(type).fieldOf("entries")).forGetter(PackOverlaysMetadata::overlays)).apply((Applicative<PackOverlaysMetadata, ?>)instance, PackOverlaysMetadata::new));
    }
    ...
    public record Entry(Range<PackVersion> format, String overlay) {
        ...
        {
            static final Codec<Holder> CODEC = RecordCodecBuilder.create(instance -> instance.group(PackVersion.Format.OVERLAY_CODEC.forGetter(Holder::format), ((MapCodec)Codec.STRING.validate(PackOverlaysMetadata::validate).fieldOf("directory")).forGetter(Holder::overlay)).apply((Applicative<Holder, ?>)instance, Holder::new));
