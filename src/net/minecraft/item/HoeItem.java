...
extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (predicate.test(context)) {
            ...
            lv.playSound((Entity)lv3, lv2, SoundEvents.ITEM_HOE_TILL, SoundCategory.BLOCKS, 1.0f, 1.0f);
            if (!lv.isClient()) {
                consumer.accept(context);
                if (lv3 != null) {
                    context.getStack().damage(1, (LivingEntity)lv3, context.getHand().getEquipmentSlot());
                }
            }
