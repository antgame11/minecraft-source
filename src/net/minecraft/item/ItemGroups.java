...
public class ItemGroups {
        ...
        Registry.register(registry, OPERATOR, ItemGroup.create(ItemGroup.Row.BOTTOM, 5).displayName(Text.translatable("itemGroup.op")).icon(() -> new ItemStack(Items.COMMAND_BLOCK)).special().entries((displayContext, entries) -> {
            if (displayContext.hasPermissions()) {
                ...
                for (TestBlockMode lv : TestBlockMode.values()) {
                    entries.add(TestBlock.applyBlockStateToStack(new ItemStack(Items.TEST_BLOCK), lv));
                }
                for (int i = 15; i >= 0; --i) {
    ...
    private static void addPaintings(ItemGroup.Entries entries, RegistryWrapper.WrapperLookup registries, RegistryWrapper.Impl<PaintingVariant> registryWrapper, Predicate<RegistryEntry<PaintingVariant>> filter, ItemGroup.StackVisibility stackVisibility) {
        ...
        registryWrapper.streamEntries().filter(filter).sorted(PAINTING_VARIANT_COMPARATOR).forEach(arg3 -> {
            ItemStack lv = new ItemStack(Items.PAINTING);
            lv.set(DataComponentTypes.PAINTING_VARIANT, arg3);
            entries.add(lv, stackVisibility);
        });
    }
